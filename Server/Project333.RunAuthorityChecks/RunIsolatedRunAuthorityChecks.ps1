...
$environment = @{
    ...
    # Expanded synthetic-account suite; applies only to this isolated child server.
    PROJECT333_AUTH_RATE_LIMIT_PER_WINDOW = '60'
    PROJECT333_ACCOUNT_RATE_LIMIT_PER_WINDOW = '600'
    PROJECT333_BATTLE_CONNECT_RATE_LIMIT_PER_WINDOW = '100'
...
function Assert-TombstoneWithoutDatabase([string]$Body) {
    try {
        $request = @{ Method = 'Post'; Uri = "http://127.0.0.1:$httpPort/runs/sync-local-record"; TimeoutSec = 5 }
        if ($null -ne $Body -and $Body.Length -gt 0) { $request.Body = $Body; $request.ContentType = 'application/json' }
        Invoke-WebRequest @request -UseBasicParsing | Out-Null
        throw 'No-DB tombstone unexpectedly succeeded.'
    } catch {
        $response = $_.Exception.Response
        if ($null -eq $response -or [int]$response.StatusCode -ne 410) { throw }
        ...
        if ($payload.error.code -ne 'client_authoritative_run_result_removed') { throw 'No-DB tombstone returned the wrong error code.' }
    }
}

try {
    $harnessArtifacts = Join-Path $runRoot 'HarnessArtifacts'
    & dotnet build (Join-Path $PSScriptRoot 'RunAuthorityChecks.csproj') --configuration Release `
    ...
    $harnessDll = Join-Path $harnessArtifacts 'bin\RunAuthorityChecks\release\RunAuthorityChecks.dll'
    ...
    while (-not $harness.WaitForExit(20000)) {
        if ($harnessTimer.Elapsed.TotalSeconds -ge 140) {
    ...
    # Stop/restart only our isolated process, then retry committed operations with
    # synthetic credentials saved by the harness. No live server or wallet is used.
    Stop-Process -Id $server.Id
    $server.WaitForExit()
    $server = Start-Process dotnet -ArgumentList @('"' + $serverDll + '"') -WorkingDirectory $serverOutputPath -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $runRoot 'restart-server.out.log') -RedirectStandardError (Join-Path $runRoot 'restart-server.err.log')
    Wait-TestServer $server
    & dotnet $harnessDll --replay-account-operations *> (Join-Path $runRoot 'restart-operation-checks.log')
    if ($LASTEXITCODE -ne 0) { throw 'Account operation replay after restart failed.' }
    Get-Content -LiteralPath (Join-Path $runRoot 'restart-operation-checks.log')

    # The removed route must remain a tombstone even when account persistence is absent.
    Stop-Process -Id $server.Id
    $server.WaitForExit()
    $server = $null
    [Environment]::SetEnvironmentVariable('PROJECT333_DB_CONNECTION', $null, 'Process')
    $server = Start-Process dotnet -ArgumentList @('"' + $serverDll + '"', '--PROJECT333_DB_CONNECTION=') `
        -WorkingDirectory $serverOutputPath -WindowStyle Hidden -PassThru `
        -RedirectStandardOutput (Join-Path $runRoot 'no-db-server.out.log') `
        -RedirectStandardError (Join-Path $runRoot 'no-db-server.err.log')
    Wait-TestServer $server
    foreach ($body in @('{"wins":33,"losses":0}', '{"wins":0,"losses":3}', '{malformed', '')) {
        Assert-TombstoneWithoutDatabase $body
    }
    'PASS: four valid/malformed/empty uploads return the same HTTP 410 tombstone with no DB configured.' |
        Tee-Object -FilePath (Join-Path $runRoot 'no-db-checks.log')
} finally {
