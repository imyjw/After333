...
if ([string]::IsNullOrWhiteSpace($DatabaseConnection)) {
    throw "RunPublishedServer_Production.ps1 requires -DatabaseConnection. Production-like PvP must not run without PostgreSQL."
}

if ([string]::IsNullOrWhiteSpace($RequiredClientVersion)) {
    throw "RunPublishedServer_Production.ps1 requires -RequiredClientVersion so clients can see the expected build version."
}
...
Write-Host "[After333] Published production-like server settings"
...
if ($PrintOnly) {
    Write-Host "[After333] PrintOnly was set. Published server was not started."
    return
}
