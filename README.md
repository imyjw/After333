...
**과제:** 각 클라이언트가 전투를 계산하면 서로 다른 결과가 발생할 수 있고, 조작된 요청이나 재접속 상황에서 신뢰할 상태의 기준이 불명확해집니다.
...
**과제:** 앱 종료나 모바일 네트워크 변경 이후에도 같은 계정의 진행 상태를 복원해야 합니다. 선택한 카드만 저장하면 재접속 때 제시 카드가 바뀌는 문제도 생깁니다.
...
**과제:** 중복 클릭, 요청 재시도, 광고 콜백 재전송이 발생해도 재화가 중복 지급되거나 강화 비용만 차감되어서는 안 됩니다.
...
**과제:** 카드와 특수효과가 늘어날수록 개별 기능보다 효과끼리 충돌하는 경계 조건이 중요해집니다.
...
**과제:** PC와 모바일의 화면 비율이 달라지면 타일·소환물·스탯·연출 위치가 어긋나고, 네트워크 상태를 그대로 표시하는 것만으로는 전투 과정을 이해하기 어렵습니다.
...
이 포트폴리오는 **게임 규칙과 UI 구현을 넘어, 온라인 상태 동기화와 계정 데이터의 일관성까지 연결한 개발 과정**을 소개합니다. 정식 출시 및 대규모 운영 검증은 별도 단계로 구분합니다.
